 ...
 * @returns
 */
export const isValidHexString = (maybeHex: string) =>
 ...
 * @returns
 */
export const zeroPadHex = (hex: string): string =>
  hex.length % 2 === 1 ? `0${hex}` : hex;

/**
 * Converts a hexadecimal string to a {@link Uint8Array}.
 *
 * Source: <https://stackoverflow.com/questions/38987784/how-to-convert-a-hexadecimal-string-to-uint8array-and-back-in-javascript>
 *
 * @param buffer The buffer to convert.
 * @returns
 */
export const hexToBytes = (hex: string): Uint8Array => {
  ...
  return Uint8Array.from(
    (hexNormalized.match(/.{1,2}/g) ?? []).map((byte) => parseInt(byte, 16))
  );
};
